...
MAX_HISTORY = 20
MAX_TOKENS = 1000  # Increased to handle reasoning models
...
class EmotionAwareChatbot:
    ...
    def send_message(self, user_message):
        ...
        print(f"\n[DEBUG] Sending {len(self.conversation_history)} history messages to OpenRouter...")
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                print(f"[ERROR] OpenRouter HTTP {response.status_code}: {error_body}")
            ...
            if ai_response is None:
                ...
                if reasoning:
                    print(f"[DEBUG] Using reasoning field instead of content")
                    # Extract a reasonable excerpt from reasoning
                    ai_response = reasoning[:500]  # First 500 chars of reasoning
                else:
                    ai_response = ''
            ...
            if not ai_response:
                print(f"[ERROR] Empty response from OpenRouter: {response_data}")
                return "Sorry, I got an empty response from the API. Please try again."
            ...
            error_msg = f"[ERROR] KeyError: {str(e)} — Response: {response.json() if 'response' in locals() else 'N/A'}"
            print(error_msg)
